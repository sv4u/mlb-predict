"""Centralized logging configuration for the MLB Win Probability system.
...
def setup_logging(
    *,
    ...
    verbose:
        ...
        Overridden by ``WINPROB_VERBOSE=1``.
    """
    env_level = os.environ.get("WINPROB_LOG_LEVEL", "").upper()
    if env_level:
        level = getattr(logging, env_level, logging.INFO)

    env_verbose = os.environ.get("WINPROB_VERBOSE", "")
    if verbose is None:
    ...
    env_format = os.environ.get("WINPROB_LOG_FORMAT", "").lower()
    if log_format == "auto":
        log_format = "json" if env_format == "json" else "human"

    log_dir_path = Path(os.environ.get("WINPROB_LOG_DIR", str(log_dir or _DEFAULT_LOG_DIR)))
