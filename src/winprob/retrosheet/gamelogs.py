...
GAMELOG_COLUMNS = [
    "date","game_num","day_of_week","visiting_team","visiting_team_league","visiting_team_game_num",
    "home_team","home_team_league","home_team_game_num","visiting_score","home_score","num_outs",
    "day_night","completion_info","forfeit_info","protest_info","park_id","attendance",
    "time_of_game_minutes","visiting_line_score","home_line_score",
    "visiting_abs","visiting_hits","visiting_doubles","visiting_triples","visiting_homeruns",
    "visiting_rbi","visiting_sac_hits","visiting_sac_flies","visiting_hbp","visiting_bb",
    "visiting_iw","visiting_k","visiting_sb","visiting_cs","visiting_gdp","visiting_ci",
    "visiting_lob","visiting_pitchers_used","visiting_individual_er","visiting_er","visiting_wp",
    "visiting_balks","visiting_po","visiting_assists","visiting_errors","visiting_pb","visiting_dp",
    "visiting_tp","home_abs","home_hits","home_doubles","home_triples","home_homeruns","home_rbi",
    "home_sac_hits","home_sac_flies","home_hbp","home_bb","home_iw","home_k","home_sb","home_cs",
    "home_gdp","home_ci","home_lob","home_pitchers_used","home_individual_er","home_er","home_wp",
    "home_balks","home_po","home_assists","home_errors","home_pb","home_dp","home_tp",
    "ump_home_id","ump_home_name","ump_first_id","ump_first_name","ump_second_id","ump_second_name",
    "ump_third_id","ump_third_name","ump_lf_id","ump_lf_name","ump_rf_id","ump_rf_name",
    "visiting_manager_id","visiting_manager_name","home_manager_id","home_manager_name",
    "winning_pitcher_id","winning_pitcher_name","losing_pitcher_id","losing_pitcher_name",
    "save_pitcher_id","save_pitcher_name","game_winning_rbi_id","game_winning_rbi_name",
...
async def download_gamelog_txt(
    *,
    ...
    if out_path.exists() and not refresh:
        ...
        return {"season": season, "path": str(out_path), "sha256": sha256_bytes(data), "cached": True, "source_used": "cache", "url_used": None, "fallback_reason": None}
    ...
    try:
        ...
        return {"season": season, "path": str(out_path), "sha256": sha256_bytes(data), "cached": False, "source_used": primary, "url_used": url_used, "fallback_reason": None}
    except Exception as e_primary:
        if not source.enable_fallback:
            raise
        data, url_used = await attempt(fallback)
        out_path.write_bytes(data)
        return {"season": season, "path": str(out_path), "sha256": sha256_bytes(data), "cached": False, "source_used": fallback, "url_used": url_used, "fallback_reason": f"{type(e_primary).__name__}: {e_primary}"}
...
def parse_gamelog_txt(path: Path) -> pd.DataFrame:
    ...
    if df.shape[1] != len(GAMELOG_COLUMNS):
        raise ValueError(f"Unexpected column count: got {df.shape[1]} expected {len(GAMELOG_COLUMNS)}")
